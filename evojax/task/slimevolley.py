...
PIXEL_MODE = True 
PIXEL_SCALE = 2 # first render at multiple of Pixel Obs resolution, then downscale. Looks better.
...
def initBaselinePolicyParams():
    ...
    weight = weight.reshape(nGameOutput+nRecurrentState, nGameInput+nGameOutput+nRecurrentState)
...
def initParticleState(x: jnp.float32, y: jnp.float32, vx: jnp.float32, vy: jnp.float32, r: jnp.float32):
    return ParticleState(jnp.float32(x), jnp.float32(y), jnp.float32(x), jnp.float32(y),
        jnp.float32(vx), jnp.float32(vy), jnp.float32(r))

@dataclass
class AgentState(object):
    direction: jnp.int32 # -1 means left, 1 means right player for symmetry.
    x: jnp.float32
    y: jnp.float32
    r: jnp.float32
    vx: jnp.float32
...
class GameState(object):
    ...
    action_left_flag: jnp.int32 # if 1, then use the action populated in GameState
...
class Particle:
    ...
    def display(self, canvas):
        return circle(canvas, toX(float(self.p.x)), toY(float(self.p.y)), toP(float(self.p.r)), color=self.c)
    def move(self):
        self.p = ParticleState(self.p.x+self.p.vx*TIMESTEP, self.p.y+self.p.vy*TIMESTEP,
                                                    self.p.x, self.p.y, self.p.vx, self.p.vy, r=self.p.r)
    def applyAcceleration(self, ax, ay):
        self.p = ParticleState(self.p.x, self.p.y,
                                                    self.p.prev_x, self.p.prev_y, self.p.vx+ax*TIMESTEP, self.p.vy+ay*TIMESTEP, r=self.p.r)
    def checkEdges(self):
        oldp = self.p
        ...
        newx = jnp.where(oldp.x<=(oldp.r-REF_W/2), oldp.r-REF_W/2+NUDGE*TIMESTEP, newx)
        newvx = jnp.where(oldp.x<=(oldp.r-REF_W/2), oldp.vx*(-FRICTION), newvx)

        newx = jnp.where(oldp.x >= (REF_W/2-oldp.r), REF_W/2-oldp.r-NUDGE*TIMESTEP, newx)
        newvx = jnp.where(oldp.x >= (REF_W/2-oldp.r), oldp.vx*(-FRICTION), newvx)

        return_value = jnp.where(oldp.y<=(oldp.r+REF_U), 1, 0)

        newy = jnp.where(oldp.y<=(oldp.r+REF_U), oldp.r+REF_U+NUDGE*TIMESTEP, newy)
        newvy = jnp.where(oldp.y<=(oldp.r+REF_U), oldp.vy*(-FRICTION), newvy)

        newy = jnp.where(oldp.y >= (REF_H-oldp.r), REF_H-oldp.r-NUDGE*TIMESTEP, newy)
        newvy = jnp.where(oldp.y >= (REF_H-oldp.r), oldp.vy*(-FRICTION), newvy)

        # fence:

        newx = jnp.where((oldp.x <= (REF_WALL_WIDTH/2+oldp.r)) & (oldp.prev_x > (REF_WALL_WIDTH/2+oldp.r)) & (oldp.y <= REF_WALL_HEIGHT),
                                         REF_WALL_WIDTH/2+oldp.r+NUDGE*TIMESTEP, newx)
        newvx = jnp.where((oldp.x <= (REF_WALL_WIDTH/2+oldp.r)) & (oldp.prev_x > (REF_WALL_WIDTH/2+oldp.r)) & (oldp.y <= REF_WALL_HEIGHT),
        ...
        self.p = ParticleState(newx, newy, newpx, newpy, newvx, newvy, r=oldp.r)
    ...
    def bounce(self, p): # bounce two balls that have collided (this and that)
        ...
        for i in range(15): # this was a while loop in the orig code, but most cases has less than this number of cases.
        ...
        return ParticleState(x=new_x, y=new_y,
                                                 prev_x=oldp.prev_x, prev_y=oldp.prev_y, vx=ux + p.vx, vy=uy + p.vy, r=oldp.r)
    def bounceIfColliding(self, p):
        ...
        self.p = ParticleState(x=newx, y=newy, prev_x=newprev_x, prev_y=newprev_y, vx=newvx, vy=newvy, r=self.p.r)
    def limitSpeed(self, minSpeed, maxSpeed):
        oldp = self.p
        mag2 = oldp.vx*oldp.vx+oldp.vy*oldp.vy
        mag = jnp.sqrt(mag2)
        maxFactor = maxSpeed/mag
        minFactor = minSpeed/mag
        ...
        #newvx = jnp.where(mag2 < (minSpeed*minSpeed), oldp.vx*minFactor, oldp.vx)
        #newvy = jnp.where(mag2 < (minSpeed*minSpeed), oldp.vy*minFactor, oldp.vy)

        self.p = ParticleState(x=oldp.x, y=oldp.y, prev_x=oldp.prev_x, prev_y=oldp.prev_y,
...
class Agent:
    ...
    def setAction(self, action):
        ...
        new_desired_vx = jnp.where(forward & (1-backward), -PLAYER_SPEED_X, new_desired_vx)
        new_desired_vx = jnp.where(backward & (1-forward), PLAYER_SPEED_X, new_desired_vx)
        
        new_desired_vy = jnp.where(jump, PLAYER_SPEED_Y, new_desired_vy)

        p = self.p
        self.p = AgentState(p.direction, p.x, p.y, p.r, p.vx, p.vy, new_desired_vx, new_desired_vy, p.life)
    def move(self):
        p = self.p
        self.p = AgentState(p.direction, p.x+p.vx*TIMESTEP, p.y+p.vy*TIMESTEP,
                                                p.r, p.vx, p.vy, p.desired_vx, p.desired_vy, p.life)
    def update(self):
        p = self.p
        ...
        self.p = AgentState(p.direction, p.x, p.y, p.r, new_vx, new_vy, p.desired_vx, p.desired_vy, p.life)
        ...
        new_x = jnp.where(p.x*p.direction <= (REF_WALL_WIDTH/2+p.r), p.direction*(REF_WALL_WIDTH/2+p.r), new_x)
        
        new_vx = jnp.where(p.x*p.direction >= (REF_W/2-p.r), 0, new_vx)
        new_x = jnp.where(p.x*p.direction >= (REF_W/2-p.r), p.direction*(REF_W/2-p.r), new_x)
        
        self.p = AgentState(p.direction, new_x, new_y, p.r, new_vx, new_vy, p.desired_vx, p.desired_vy, p.life)
    ...
    def updateLife(self, result):
        ...
        self.p = AgentState(p.direction, p.x, p.y, p.r, p.vx, p.vy, p.desired_vx, p.desired_vy, new_life)
    ...
    def display(self, canvas, ball_x, ball_y):
        ...
        for i in range(1, num_lives):
            canvas = circle(canvas, toX(direction*(REF_W/2+0.5-i*2.)), WINDOW_HEIGHT-toY(1.5), toP(0.5), color=COIN_COLOR)
...
def baselinePolicy(obs: jnp.ndarray, state: jnp.ndarray, params: BaselinePolicyParams):
    ...
    nGameInput = 8 # 8 states for agent that this policy cares about (ignores last 4)
    nGameOutput = 3 # 3 buttons (forward, backward, jump)
    nRecurrentState = 4 # extra recurrent states for feedback.
    weight = params.w
    bias = params.b
...
class Game:
    ...
    def reset(self, gameState):
        ...
        fenceStubParticle = initParticleState(0, REF_WALL_HEIGHT, 0, 0, REF_WALL_WIDTH/2)
    ...
    def setAction(self):
        ...
        self.hidden_left, action_left = baselinePolicy(obs_left, self.hidden_left, self.baselineParams)
        self.hidden_right, action_right = baselinePolicy(obs_right, self.hidden_right, self.baselineParams)
        ...
        action_right = jnp.where(self.action_right_flag, self.action_right, action_right)
        self.agent_left.setAction(action_left)
        self.agent_right.setAction(action_right)
    def getGameState(self):
        return GameState(self.ball.p, self.agent_left.p, self.agent_right.p, self.hidden_left, self.hidden_right,
                                         self.action_left_flag, self.action_left, self.action_right_flag, self.action_right)
    def step(self):
        """ main game loop """
    ...
    def display(self):
        ...
        #canvas = upsize_image(canvas)
        return canvas


def initGameState(ball_vx, ball_vy):
    # use numpy random for now.
    #np_random = np.random
    #ball_vx = np_random.uniform(low=-20, high=20)
    #ball_vy = np_random.uniform(low=10, high=25)
    ...
    action_right_flag = jnp.int32(1) # right is the external agent being trained
    action_right = jnp.array([0, 0, 1], dtype=jnp.float32)
    return GameState(ball, agent_left, agent_right, hidden_left, hidden_right,
                                     action_left_flag, action_left, action_right_flag, action_right)
...
def newMatch(prevGameState: GameState, ball_vx, ball_vy) -> GameState:
    ...
    ball = initParticleState(0, REF_W/4, ball_vx, ball_vy, 0.5)
    p = prevGameState
    return GameState(ball, p.agent_left, p.agent_right, p.hidden_left, p.hidden_right,
                                     p.action_left_flag, p.action_left, p.action_right_flag, p.action_right)
...
def detect_done(game_state: GameState):
    result = jnp.bitwise_or(game_state.agent_left.life <= 0, game_state.agent_right.life <= 0)
    return result
...
class SlimeVolley(VectorizedTask):
    ...
    def __init__(self,
        ...
        def step_fn(state, action):
            ...
            cur_state, reward, obs = update_state(action=action, game_state=state.game_state, key=key)
            steps = state.steps + 1
            done_test = jnp.bitwise_or(detect_done(cur_state), steps >= max_steps)
            done = jnp.where(self.test, done_test, steps >= max_steps) # during training, go for all 3000 steps.
